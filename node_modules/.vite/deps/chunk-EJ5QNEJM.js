...
function validate_snippet(snippet_fn, $$props) {
  var _a;
  if (((_a = $$props == null ? void 0 : $$props.$$slots) == null ? void 0 : _a.default) && typeof $$props.$$slots.default !== "boolean" || snippet_fn && snippet_fn[snippet_symbol] !== true) {
...
function state_frozen_invalid_argument() {
  if (DEV) {
...
%cThe \`${attribute}\` attribute on \`${html}\` changed its value between server and client renders. The client value, \`${value}\`, will be ignored in favour of the server value`, bold2, normal2);
  } else {
...
%c${location ? `The value of an \`{@html ...}\` block ${location} changed between server and client renders. The client value will be ignored in favour of the server value` : "The value of an `{@html ...}` block changed between server and client renders. The client value will be ignored in favour of the server value"}`, bold2, normal2);
  } else {
...
%c${location ? `Hydration failed because the initial UI does not match what was rendered on the server. The error occurred near ${location}` : "Hydration failed because the initial UI does not match what was rendered on the server"}`, bold2, normal2);
  } else {
...
%cTried to unmount a component that was not mounted`, bold2, normal2);
  } else {
...
%c${parent} passed a value to ${child} with \`bind:\`, but the value is owned by ${owner}. Consider creating a binding between ${owner} and ${parent}`, bold2, normal2);
  } else {
...
%cReactive \`$state(...)\` proxies and the values they proxy have different identities. Because of this, comparisons with \`${operator}\` will produce unexpected results. Consider using \`$state.is(a, b)\` instead`, bold2, normal2);
  } else {
...
//# sourceMappingURL=chunk-EJ5QNEJM.js.map
